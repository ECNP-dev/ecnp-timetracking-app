...
// - Universal OneDrive logs detection + override
// - Monthly subfolders for logs & reports
// - Per-user daily CSV (YYYY-MM-DD__username.csv)
// - Tasks & Monthly Excel report IPC
// - Working-hours IPC (per user)
// - Tray + mini window (#/tray) with state-aware icon (green/gray)
// - Global shortcut Ctrl+Alt+L toggle (start/pause/resume/stop)
// - Minimize-to-tray (close hides app)
// - Launch in tray mode at startup (--tray) via Windows Startup shortcut
...
function createMainWindow() {
  ...
  // Minimize-to-tray behavior (close hides)
...
app.whenReady().then(() => {
  ...
  if (launchedInTray) openTrayWindow();
...
async function generateMonthlyReport({ year, month }) {
  ...
  const monthDir = path.join(dir, `${yyyy}-${mm}`);
  ensureDirSync(monthDir);
...
function whFileForUser(user) {
  const safe = (user || "unknown").replace(/[^a-zA-Z0-9_-]/g, "_");
  return path.join(effectiveLogsDir(), `working-hours__${safe}.json`);
}
ipcMain.handle("settings:getWorkingHours", async (_e, { user }) => {
  try {
    const txt = await fsp.readFile(whFileForUser(user), "utf8");
...
ipcMain.handle("settings:saveWorkingHours", async (_e, { user, data }) => {
  await fsp.writeFile(whFileForUser(user), JSON.stringify(data, null, 2), "utf8");
  return true;
});
