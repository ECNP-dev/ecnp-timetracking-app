...
// src/App.jsx — Dual login (Microsoft + name), admin="admin",
// tasks, multi-task timer, divide-evenly (8h),
// daily CSV logs, monthly Excel report, Settings page, refreshed UI.
...
const GearIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden>
    ...
    />
    <path
      d="M19.4 15a7.97 7.97 0 0 0 .1-2 8 8 0 0 0-.1-2l2.1-1.6-2-3.4-2.5.7A8.1 8.1 0 0 0 15 5.1L14.3 3h-4.6L9 5.1A8.03 8.03 0 0 0 7 6.7l-2.5-.7-2 3.4 2.1 1.6a7.66 7.66 0 0 0 0 4.1l-2.1 1.6 2 3.4 2.5-.7a8.1 8.1 0 0 0 2 1.6l.7 2.1h4.6l.7-2.1a8.1 8.1 0 0 0 2-1.6l2.5.7 2-3.4-2.1-1.6z"
      stroke="#444"
      strokeWidth="1.8"
    />
  </svg>
...
export default function App() {
  ...
  const userInitials = useMemo(
    () => initialsFor(storedUser || username),
    [storedUser, username]
  );
  ...
  const handleMicrosoftLogin = () => {
    msalInstance.loginRedirect(loginRequest); // Redirect navigation (MSAL docs)[3](blob:https://outlook.office.com/e53eb52a-2806-4f90-b590-c40202f7b3fa)
  };
  ...
  const saveTaskList = async () => {
    if (!window.ecnp?.saveTasks) {
      alert("Cannot save tasks: preload API missing.");
      return;
    }
    try {
  ...
  async function appendElapsed(ms) {
    if (!window.ecnp?.appendDaily) {
      alert("Cannot write logs: preload API missing.");
      return;
    }
  ...
  /* ----------------------- Divide evenly: 8h among selected ----------------------- */
  const divideEvenly = async () => {
    ...
    if (selected.length === 0) {
      alert("Select tasks to divide among.");
      return;
    }
    const total = 8 * 60;
  ...
  const generateMonthly = async () => {
    if (!window.ecnp?.generateMonthly) {
      alert("Cannot generate report: preload API missing.");
      return;
    }
    if (!/^\d{4}-\d{2}$/.test(reportMonth)) {
      alert("Use YYYY-MM format.");
      return;
    }
  ...
  const openLogsDir = async () => {
    if (!window.ecnp?.openLogsDir) {
      alert("Cannot open folder: preload API missing.");
      return;
    }
  ...
  const testWrite = async () => {
    if (!window.ecnp?.writeTestLog) {
      alert("Cannot test write: preload API missing.");
      return;
    }
    try {
  ...
  const overrideLogs = async () => {
    if (!isAdmin) {
      setSettingsMsg("Only admin can override logs folder.");
      return;
    }
    if (!overridePath.trim()) {
      setSettingsMsg("Please enter a valid path.");
      return;
    }
    if (!window.ecnp?.overrideLogsDir) {
      setSettingsMsg("Override API missing.");
      return;
    }
    try {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <h3 id="ms-title" style={sectionTitleStyle}>
            ...
            <p style={{ ...mutedStyle, fontSize: 13 }}>
              Having issues with Microsoft sign‑in? Use your name instead on the
              right — you can switch later.
            </p>
          </section>
          ...
          >
            <h3 id="name-title" style={sectionTitleStyle}>
            ...
            >
              <label htmlFor="name-input" style={mutedStyle}>
                Your name
              </label>
              <input
                id="name-input"
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" ? handleNameLogin() : null
                }
                aria-invalid={!!nameErr}
                aria-describedby={nameErr ? "name-error" : undefined}
              />
              {nameErr && (
                <span id="name-error" style={{ color: "#b42318", fontSize: 13 }}>
                  {nameErr}
                </span>
              )}
            </div>
            <div className="actions" style={{ display: "flex", gap: 10 }}>
              <button className="btn ok" style={btnOk} onClick={handleNameLogin}>
                Continue
              </button>
              <button
            ...
            <p style={{ ...mutedStyle, fontSize: 13 }}>
              Your name is saved locally so you don’t have to type it again next
              time.
            </p>
          </section>
        </div>
    ...
    >
      ...
      <header className="topbar" style={topbarStyle}>
        ...
        >
          ...
          <div className="initial" aria-hidden="true" style={avatarStyle}>
            {userInitials}
          </div>
          <div>
      ...
      {page === "settings" ? (
        <section className="panel" style={panelStyle} aria-label="Settings">
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 13, wordBreak: "break-all" }}>
                {logsDir || "Not available yet"}
              </div>
            </div>

            <div className="actions" style={{ display: "flex", gap: 10 }}>
              <button style={btnOutline} onClick={openLogsDir}>
                Open folder
              </button>
              <button style={btnOutline} onClick={testWrite}>
                Test write
              </button>
            </div>
          </div>
          ...
          {isAdmin && (
            <>
              ...
              <div className="field" style={{ display: "flex", gap: 10 }}>
                ...
                />
                <button style={btnOk} onClick={overrideLogs}>
                  Save override
                </button>
              </div>
          ...
          <div style={{ marginTop: 16 }}>
            <button style={btnOutline} onClick={() => setPage("main")}>
              Back
            </button>
          </div>
        </section>
        ...
        <>
              ...
              >
                <div className="field" style={{ flex: 1 }}>
                  ...
                  <div className="row" style={{ display: "flex", gap: 10 }}>
                    ...
                    />
                    <button style={btnOutline} onClick={addTask}>
                      Add
                    </button>
                  </div>
                </div>
                <div>
                  <button style={btnOk} onClick={saveTaskList}>
                    Save tasks
                  </button>
                </div>
              </div>
          ...
          >
            ...
            >
              <h3 id="work-title" style={sectionTitleStyle}>
                Today
              </h3>
              ...
              >
                {!isRunning && (
                  <button className="btn ok" style={btnOk} onClick={startTimer}>
                    Start
                  </button>
                )}
                {isRunning && (
            ...
            <aside className="panel" style={panelStyle}>
              ...
              <p style={{ ...mutedStyle, fontSize: 13, marginTop: 8 }}>
                The report is saved in the shared logs folder and includes a
                monthly task summary and per-user breakdown.
              </p>
            </aside>
          </div>
        </>
